...
const Cursor: React.FC<CursorProps> = ({ triger }) => {
        ...
        gsap.timeline({
            ...
            onUpdate: () => {
                if (feTurbulence.current) {
                    feTurbulence.current.setAttribute('baseFrequency', primitiveValues.turbulence)
                }
            },
            onComplete: () => {
                if (cursorInnerRef.current) {
    ...
    const render = useCallback(() => {
        ...
        for (const key in renderedStyles) {
            renderedStyles[key as 'tx' | 'ty' | 'radius'].previous = MathUtils.lerp(
    ...
    const onMouseMove = useCallback(() => {
        renderedStyles['tx'].previous = renderedStyles['tx'].current = docX - size.width / 2;
        renderedStyles['ty'].previous = renderedStyles['ty'].previous = docY - size.height / 2;
