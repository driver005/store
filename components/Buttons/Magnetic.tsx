...
export const ButtonMagnetic: React.FC<ButtonProps> = ({ label, onClick, className, height = 80, type = 'button', scale = true, loaded = true }) => {
    ...
    const render = useCallback(() => {
        renderedStyles['tx'].current = (docX + window.scrollX - (button.current.getBoundingClientRect().left + button.current.getBoundingClientRect().width / 2)) * .3;
        renderedStyles['ty'].current = (docY + window.scrollY - (button.current.getBoundingClientRect().top + button.current.getBoundingClientRect().height / 2)) * .3;

        for (const key in renderedStyles) {
            renderedStyles[key as 'tx' | 'ty' | 'scale'].previous = MathUtils.lerp(
        ...
        if (button.current && buttonText.current && buttonDeco.current) {
            ...
            buttonText.current.style.transform = `translate3d(${-renderedStyles['tx'].previous * 0.2}px, ${-renderedStyles['ty'].previous * 0.2}px, 0)`;
